...
class ShowPathDetailViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let dummy = DetailPath(stations: ["외대앞"], type: .bus, startStation: "테크노마트앞.강변역", startNum: 2224, arriveStation: "강변역", stationNum: 1, time: 3, open: false)
        items.append(dummy)
...
extension ShowPathDetailViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if items[section].open == true {
            return 2 + items[section].stations.count
        } else {
            return 2
        }
    }
